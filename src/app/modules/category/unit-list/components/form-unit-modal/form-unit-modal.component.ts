...
@Component({
  selector: 'app-form-unit-modal',
  templateUrl: './form-unit-modal.component.html',
  styleUrls: ['./form-unit-modal.component.scss']
})
export class FormUnitModalComponent implements OnInit {
  ...
  private initForm() {
    this.formGroup = new FormGroup({
      maDVT: new FormControl('', Validators.required),
      tenDVT: new FormControl('', Validators.required),
  ...
  public onCreate() {
    ...
    if (this.formGroup.valid) {
      ...
      // Call api
      this.api.post(`/don-vi-tinh-create`, bodyParam).subscribe(
        res => {
          if (res) {
            swal({
              icon: "success",
