...
import { FormDistrictModalComponent } from './components/form-district-modal/form-district-modal.component';
...
export class DistrictListComponent implements OnInit {
  ...
  @ViewChild('formExpressDelivery', { static: false }) formExpressDelivery: FormDistrictModalComponent;
  ...
  constructor(
    ...
    this.dataTable.columns = [
      {
        field: 'tenQuanHuyen',
        sortField: 'tenQuanHuyen',
        title: 'Tên quận huyện',
        width: '200px',
      },
      {
        field: 'maQuanHuyen',
        sortField: 'maQuanHuyen',
        title: 'Mã quận huyện',
        width: '200px',
      },
      {
  ...
  public getList() {
    ...
    this.api.get('/api/quan-huyen-list', query).subscribe(res => {
      if (res) {
   ...
   */
   public onDelete(Id) {
    this.api.delete(`/api/quan-huyen-delete?QuanHuyenId=${Id }`)
      .subscribe((res: any) => {
        if (res?.isSuccess) {
          swal({
            icon: "success",
