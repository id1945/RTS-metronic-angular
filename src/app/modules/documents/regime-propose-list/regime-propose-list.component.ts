...
import { RegimeProposeFormModalComponent } from './components/regime-propose-form-modal/regime-propose-form-modal.component';
import { DataTable, Columns } from 'src/app/_metronic/shared/shared/models/dataTable';

@Component({
  selector: 'app-regime-propose-list',
  templateUrl: './regime-propose-list.component.html',
  styleUrls: ['./regime-propose-list.component.scss']
})
export class RegimeProposeListComponent implements OnInit {
  ...
  @ViewChild('regimeProposeForm', { static: false }) regimeProposeForm: RegimeProposeFormModalComponent;
  ...
  public getList() {
    ...
    this.api.get('/api/hoso-vanbandinhche-list', query).subscribe(res => {
      if (res) {
  ...
  public getTrangThaiOptions() {
    this.api.get('/vanban-dinhche-danhmuc/trang-thai')
  ...
  public zipFile(id) {
    ...
    this.api.get('/vanban-dinhche-misc/zip-all-files', query, header).subscribe(res => {
      /* Get blob */
  ...
  public showDetail(item) {
    this.router.navigate(['documents', 'regime-propose-detail', item?.hoSoId]);
  }
}
