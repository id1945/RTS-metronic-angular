...
@Component({
  selector: 'app-approve-regime-propose-modal',
  templateUrl: './approve-regime-propose-modal.component.html',
  styleUrls: ['./approve-regime-propose-modal.component.scss']
})
export class ApproveRegimeProposeModalComponent implements OnInit {
  ...
  onSubmitApprove() {
    ...
    if (this.mode === 'approve') {
      ...
      requestFinal = this.api.post('/api/hoso-vanbandinhche-create/phe-duyet', bodyParams);
    } else if (this.mode === 'reject') {
      title = "Bạn đã từ chối thành công!";
      requestFinal = this.api.post('/api/hoso-vanbandinhche-create/tu-choi', bodyParams);
    } 
